...
public class User_Statistics extends JFrame {
	...
	public User_Statistics() {
		...
		btn_viewStat.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				load_table(model, "SELECT action_ID, COUNT(*) FROM log WHERE ISBN > 0 AND action_type = 'Borrow' AND action_date between date('" + from + "') and date('" + to + "') + 1 GROUP BY action_ID HAVING COUNT(*) > 0 ORDER BY 2 DESC", load_progressbar);
			}
		});
		...
		btn_reset.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				load_table(model, "SELECT action_ID, COUNT(*) FROM log WHERE ISBN > 0 AND action_type = 'Borrow' GROUP BY action_ID HAVING COUNT(*) > 0 ORDER BY 2 DESC", load_progressbar);
			}
		});
		panel.add(btn_reset);
		...
		btn_graph.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for(int i = 0; i < table.getRowCount(); i++) {
					User requested_user = db.load_User(String.valueOf(model.getValueAt(i, 0)), load_progressbar);
					barDataset.setValue(Integer.parseInt(String.valueOf(model.getValueAt(i, 1))), "User", requested_user.name);
				}
				
				JFreeChart chart = ChartFactory.createBarChart("전체 기간의 다독자", "ID", "대출 횟수", barDataset, PlotOrientation.HORIZONTAL, false, true, false);;
				...
				chart = ChartFactory.createBarChart(from + "부터 " + to + "까지의 다독자", "ID", "대출 횟수", barDataset, PlotOrientation.HORIZONTAL, false, true, false); }
	...
	try {
		...
		while(rs.next()){
			String name = rs.getString("action_ID");
			int count = rs.getInt(2);

			String []list = {name, Integer.toString(count)};
			model.addRow(list);
	   }
